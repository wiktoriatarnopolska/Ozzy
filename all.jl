...
function plot_all(data)
    ...
    p = plot(title = _format_metric(data.metric) * incl_text, legend = :topleft)#, ylims = (0, 0.015))
...
function run_all(INCLINATION; KWARGS...)
    ...
    m_n3 = JohannsenMetric(M = 1.0, a = 0.998, α13 = -0.35)
    ...
    # fergus 20: okay but kinda wobbly close to peak?
    m_k3 = JohannsenMetric(M = 1.0, a = 0.998, α13 = 0.0)
    ...
    p_total = plot(pn1, pn2, pn3, pk1, pk2, pk3, pp1, pp2, pp3, layout = grid(3, 3), size = (1100, 1100))
    display(p_total)
    p_total
end
